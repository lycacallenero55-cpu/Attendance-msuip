...
// Subject operations
export const fetchSubjects = async (program?: string, year_level?: string, semester?: string): Promise<Subject[]> => {
  try {
    let query = supabase
      .from('subjects')
      .select('*')
...
export const updateSubject = async (id: number, subjectData: Partial<Subject>): Promise<Subject> => {
  const { data, error } = await supabase
    .from('subjects')
    .update(subjectData as any)
    .eq('id', id)
    .select()
    .single();
